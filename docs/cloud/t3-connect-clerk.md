...
Local desktop development uses `doudou-code-dev://app`, while packaged builds use `doudou-code://app`. Add the
matching origin to each Clerk instance's Backend API `allowed_origins` array as well. The development
Clerk instance should only need `doudou-code-dev://app`; the production Clerk instance should only need
`doudou-code://app`. `@clerk/electron` owns the native request adapter, encrypted Clerk token persistence,
...
curl -X PATCH https://api.clerk.com/v1/instance \
  ...
  -d '{"allowed_origins":["doudou-code://app"]}'
```
...
The production macOS bundle ID is `io.github.sipiyou39.doudoucode`. To enable native passkeys:

1. Create an explicit macOS App ID for `io.github.sipiyou39.doudoucode` in the Apple Developer portal and enable
   **Associated Domains**.
...
4. Confirm Clerk serves `https://<frontend-api>/.well-known/apple-app-site-association` and that
   `webcredentials.apps` contains `<TEAM_ID>.io.github.sipiyou39.doudoucode`.
...
T3CODE_MACOS_PROVISIONING_PROFILE=/absolute/path/to/doudou-code.provisionprofile
...
T3CODE_PORT=13773 \
  "/Applications/Doudou Code.app/Contents/MacOS/Doudou Code"
```
...
```sh
codesign --verify --deep --strict "/Applications/Doudou Code.app"
codesign -d --entitlements :- "/Applications/Doudou Code.app"
```
