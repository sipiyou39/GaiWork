...
This guide is for people who want to use more than one Codex account in Doudou Code.
...
In Doudou Code Settings, name it something obvious:
...
In Doudou Code Settings, add another Codex provider:
...
Doudou Code shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Doudou Code treats it
