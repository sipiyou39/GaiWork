...
const logManualMacOsExportInstructions = Effect.fn("iconExport.logManualMacOsExportInstructions")(
  function* () {
    yield* Console.warn(
      [
        "macOS icon assets are intentionally not generated by Icon Composer.",
        `Development, nightly, and stable desktop builds share ${BRAND_ASSET_PATHS.doudouCodeMacIconPng}.`,
        "See assets/README.md before replacing the source artwork.",
      ].join("\n"),
    );
  },
);
...
export const exportBrandIcons = Effect.fn("exportBrandIcons")(function* (checkOnly: boolean) {
  ...
  for (const override of DEVELOPMENT_PUBLIC_ICON_OVERRIDES) {
    const generatedSourceContents = generated.get(override.sourceRelativePath);
        ...
        yield* fs.readFile(path.join(repositoryRoot, override.sourceRelativePath)).pipe(
          Effect.mapError(
            (cause) =>
              new IconExportFileSystemError({
                operation: "read-file",
                path: override.sourceRelativePath,
                cause,
              }),
          ),
        ),
