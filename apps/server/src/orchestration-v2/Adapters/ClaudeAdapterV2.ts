    ...
    openSession: Effect.fn("ClaudeAdapterV2.openSession")(
      function* (input: ProviderAdapterV2OpenSessionInput) {
        ...
        const openQuery = Effect.fnUntraced(function* (
          ...
          const openedThisRuntime = (yield* Ref.get(openedNativeThreads)).has(nativeThreadId);
          // openedNativeThreads dies with this session runtime (idle release,
          // app restart). The persisted provider thread is the durable signal:
          // firstRunOrdinal is stamped when a turn starts, so an earlier run
          // means the native session was already opened and must be resumed —
          // a create-style open for an existing session fails the whole turn.
          const establishedByPriorRun =
            turnInput.providerThread.firstRunOrdinal !== null &&
            turnInput.providerThread.firstRunOrdinal < turnInput.runOrdinal;
          const shouldResume =
            resumeSessionAt !== undefined || openedThisRuntime || establishedByPriorRun;
          ...
          // leave the runtime believing the native session exists.
